...
describe('onMessage callback', () => {
  ...
  it('calls a callback function on valid submit', () => {
  ...
  it('does not call a callback function when status is not connect', () => {
    const callback = jest.fn();
  ...
  it('doesnt call a callback function if submit data is invalid', () => {
    const callback = jest.fn();
    ChatForm.setIsConnected(true);
    ChatForm.onMessage(callback);
  ...
  it('sets correct error states for inputs if submit data is invalid', () => {
    const callback = jest.fn();
    ChatForm.setIsConnected(true);
    ChatForm.onMessage(callback);
    elements.chatInputUser.value = '';
